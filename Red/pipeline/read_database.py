...
if flag == 1:
    ...
    save_filename_txt = 'dataset_RS.txt'
else:
    load_filename = 'DB_ssf_nonQS_100_c.mat'
    save_filename = 'dataset_nonQS.mat'
    save_filename_txt = 'dataset_nonQS.txt'
...
def format_matrix_to_str(matrix, indent="    "):
    if not isinstance(matrix, np.ndarray):
        matrix = np.array(matrix)
    lines = []
    if matrix.ndim == 0:
        lines.append(f"{indent}[ {matrix.item():12.6f} ]")
    elif matrix.ndim == 1:
        row_str = " ".join(f"{val:12.6f}" for val in matrix)
        lines.append(f"{indent}[ {row_str} ]")
    elif matrix.ndim == 2:
        for r in range(matrix.shape[0]):
            row_str = " ".join(f"{val:12.6f}" for val in matrix[r, :])
            lines.append(f"{indent}[ {row_str} ]")
    else:
        lines.append(f"{indent}{matrix}")
    return "\n".join(lines)

def save_dataset_as_txt(A_list, B_list, K_list, nx_list, nu_list, N_list, filename):
    print(f"Guardando base de datos en formato texto (.txt): {filename}...")
    with open(filename, 'w', encoding='utf-8') as f:
        f.write("============================================================\n")
        f.write("BASE DE DATOS DE CONTROL ROBUSTO - EXPORTACIÓN EN TEXTO PLANO\n")
        f.write("============================================================\n")
        f.write(f"Total de muestras: {len(A_list)}\n\n")
        ...
        for idx in range(len(A_list)):
            ...
            f.write(f"Dimensiones: nx={nx}, nu={nu}, N={N}\n")
            
            # A_list[idx] has shape (nx, nx, N)
            f.write("Matrices A (Vértices del politopo):\n")
            for v in range(N):
                f.write(f"  Vértice {v + 1}:\n")
                A_v = A_list[idx][:, :, v]
                f.write(format_matrix_to_str(A_v, indent="    ") + "\n")
            
            # B_list[idx] has shape (nx, nu, N)
            f.write("Matrices B (Vértices del politopo):\n")
            for v in range(N):
                f.write(f"  Vértice {v + 1}:\n")
                B_v = B_list[idx][:, :, v]
                f.write(format_matrix_to_str(B_v, indent="    ") + "\n")
            
            # K_list[idx] has shape (nu, nx)
            f.write("Matriz de Ganancia K:\n")
            K_mat = K_list[idx]
            f.write(format_matrix_to_str(K_mat, indent="    ") + "\n")
            
            f.write("-" * 60 + "\n\n")
    print(f"Base de datos guardada en texto plano en: {filename}")

# Guardar también en formato .txt
save_dataset_as_txt(A_data_list, B_data_list, K_data_list, labels_nx_list, labels_nu_list, labels_N_list, save_filename_txt)